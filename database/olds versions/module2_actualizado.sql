...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 14-09-2022 a las 23:22:14
...
--
DELIMITER $$
CREATE TRIGGER `inventario_actual` AFTER INSERT ON `entradas` FOR EACH ROW BEGIN
IF !EXISTS (SELECT * FROM inventario WHERE NEW.idProducto = idProducto) THEN BEGIN
INSERT INTO inventario (`totalActual`, `idProducto`) VALUES (NEW.totalEntrada, NEW.idProducto);
END;
ELSE 
UPDATE inventario SET totalActual = totalActual + NEW.totalEntrada 
...
INSERT INTO `inventario` (`idInventario`, `totalActual`, `idProducto`) VALUES
(39, 15, 1),
(40, 8, 2),
(41, 9, 8),
(42, 4, 9),
...
UPDATE inventario SET totalActual = totalActual - NEW.totalSalida 
WHERE idProducto = NEW.idProducto;
END
$$
DELIMITER ;
...
ALTER TABLE `inventario`
  ...
  ADD KEY `idProducto` (`idProducto`) INVISIBLE;
...
--
ALTER TABLE `inventario`
  MODIFY `idInventario` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=46;

--
-- AUTO_INCREMENT de la tabla `productos`
--
ALTER TABLE `productos`
  MODIFY `idProducto` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
